...
def get_master_csv_data(master_csv_path):
    data_dict = []
    with open(master_csv_path, 'r', encoding='utf-8') as f:
        csv_reader = csv.DictReader(f)
        for row in csv_reader:
...
def get_videos_info(*args, **kwargs):
    matched_output_dir = kwargs.get('matched_output_dir', os.path.join(os.getcwd(), 'outputs' ,'matched_videos'))
    unmatched_output_dir = kwargs.get('unmatched_output_dir', os.path.join(os.getcwd(), 'outputs' ,'unmatched_videos'))
    ...
    if not master_csv_path or not location:
        k_print(f'Master CSV path and location are required kwargs: master_csv_path, location', log_level='error')
        return
    
    master_data = get_master_csv_data(master_csv_path)
    ...
    while next_page:
        ...
        data += res_json['data']
        next_page = res_json['paging']['next']   
    k_print(f"Total Videos: {total_videos} - Scrapped Total Videos: {len(data)} for location: {kwargs['location']}", log_level='info')
    if len(data) != total_videos:
        k_print(f"Total video and Scrapped videos mismatched for location: {kwargs['location']}", log_level='error')
        return
    matched_csv_data = []
    unmatched_csv_data = []
    for video in data:
        name, video_url = video['name'], video['link']
        matched_data = [d for d in master_data if name.strip() in d['OWNER NAME (BANGLA)'].strip() and d['LOCATION'].strip().upper() == kwargs['location'].strip().upper()]
        if matched_data:
            data = {'code': matched_data[0]['CODE'], 'name_english': matched_data[0]['OWNER NAME (ENGLISH)'], 'name_bangla': matched_data[0]['OWNER NAME (BANGLA)'], 'location': matched_data[0]['LOCATION'], 'video_url': video_url}
            matched_csv_data.append(data)
            master_data.remove(matched_data[0])
        else:
            unmatched_csv_data.append({'video_title': name, 'video_url': video_url})

    k_print(f"Matched Videos: {len(matched_csv_data)} - Unmatched Videos: {len(unmatched_csv_data)} for location: {kwargs['location']}", log_level='info')
    with open(os.path.join(matched_output_dir, f'{location}_matched.csv'), 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['code', 'name_english', 'name_bangla', 'location', 'video_url']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
    ...
    with open(os.path.join(unmatched_output_dir, f'{location}_unmatched.csv'), 'w', newline='', encoding='utf-8') as f:
